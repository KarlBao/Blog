...
不同于Angular1与React，访问器属性(getter-setter pair)是Vue中数据响应的基石。之前写的一篇博客简单介绍过访问器属性（[戳我](https://karlbao.github.io/blog/2016/07/05/setter-getter-pair%E5%92%8Cproperty-descriptor.html)）。简言之，Vue中使用访问器属性避免了变化监测中的暴力比对过程，提高数据监测效率（当然此行为会适当增加内存消耗）。
...
以上例子在`obj`对象的`name`属性上定义了一个存取描述符，允许定义`set`和`get`两个方法。当访问`name`属性时，`get`方法会被调用。而当重新设置`name`属性的值时，`set`方法会被调用, 并且将新的值作为参数传入方法。
